...
def load_pretrained(ckpt_path, model, logger):
    logger.info(
        f"==============> Loading pretrained form {ckpt_path}...................."
    )
    ...
    agent_bias_keys = [
        ...
        if ("ah_bias" in k) or ("aw_bias" in k) or ("ha_bias" in k) or ("wa_bias" in k)
    ]
    for k in agent_bias_keys:
        ...
        agent_bias_current = (
            model.state_dict()[k].squeeze(dim=0).squeeze(dim=squeeze_dim)
        )
        if permute:
        ...
        if (num_heads1 != num_heads2) or (agent_num1 != agent_num2):
            ...
            if hw1 != hw2:
                ...
                if permute:
                    agent_bias_pretrained_resized = (
                        agent_bias_pretrained_resized.permute(0, 2, 1)
                    )
                state_dict[k] = agent_bias_pretrained_resized.unsqueeze(
                    dim=0
                ).unsqueeze(dim=squeeze_dim)

    # bicubic interpolate patch_embed.proj if not match
    patch_embed_keys = [
        k for k in state_dict.keys() if ("patch_embed" in k) and (".proj.weight" in k)
    ]
    ...
    for k in absolute_pos_embed_keys:
        ...
        if C1 != C1:
            ...
            if L1 != L2:
                ...
                S2 = int(L2**0.5)
                i, j = L1 - S1**2, L2 - S2**2
                absolute_pos_embed_pretrained_ = absolute_pos_embed_pretrained[
                    :, i:, :
                ].reshape(-1, S1, S1, C1)
                absolute_pos_embed_pretrained_ = absolute_pos_embed_pretrained_.permute(
                    0, 3, 1, 2
                )
                ...
                state_dict[k] = torch.cat(
                    [
                        absolute_pos_embed_pretrained[:, :j, :],
                        absolute_pos_embed_pretrained_resized,
                    ],
                    dim=1,
                )
