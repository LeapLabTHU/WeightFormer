...
class DiTBlock(nn.Module):
    ...
    def __init__(self, hidden_size, mlp_ratio=4.0, layer=None, **block_kwargs):
        super().__init__()
        self.norm1 = nn.LayerNorm(hidden_size, elementwise_affine=False, eps=1e-6)
        self.norm2 = nn.LayerNorm(hidden_size, elementwise_affine=False, eps=1e-6)
        self.approx_gelu = lambda: nn.GELU(approximate="tanh")
    ...
    def forward(self, x, c):
        ...
        H = W = int(N**0.5)
        x = self.norm1(x)
        x = x + gate_msa.unsqueeze(1) * self.dwc(
            modulate(x, shift_msa, scale_msa).reshape(B, H, W, C).permute(0, 3, 1, 2)
        ).flatten(2).permute(0, 2, 1)
        x = x + gate_mlp.unsqueeze(1) * self.mlp_forward(
...
class DiT(nn.Module):
    ...
    def __init__(
        ...
        self.blocks = nn.ModuleList(
            [DiTBlock(hidden_size, mlp_ratio, layer=i) for i in range(depth)]
        )
...
def DiT_B_2(**kwargs):
    return DiT(depth=15, hidden_size=768, patch_size=2, **kwargs)


def DiT_B_4(**kwargs):
    return DiT(depth=15, hidden_size=768, patch_size=4, **kwargs)


def DiT_B_8(**kwargs):
    return DiT(depth=15, hidden_size=768, patch_size=8, **kwargs)


def DiT_S_2(**kwargs):
    return DiT(depth=15, hidden_size=384, patch_size=2, **kwargs)


def DiT_S_4(**kwargs):
    return DiT(depth=15, hidden_size=384, patch_size=4, **kwargs)


def DiT_S_8(**kwargs):
    return DiT(depth=15, hidden_size=384, patch_size=8, **kwargs)
