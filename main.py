...
def parse_option():
    parser = argparse.ArgumentParser(
        "Linear-Time Global Visual Modeling without Explicit Attention", add_help=False
    )
    parser.add_argument(
        "--cfg",
        type=str,
...
def main():
    ...
    if args.use_checkpoint and hasattr(model, "set_grad_checkpointing"):
    ...
    for epoch in range(config.TRAIN.START_EPOCH, total_epochs):
        ...
        if dist.get_rank() == 0:
            wandb.log(
                {"val/acc": acc1, "val/loss": loss, "train/loss": train_loss},
                step=epoch,
            )
